          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-start mb-12 print:mb-6">
               <div className="flex-1">
                  ...
                  <h2 className="text-2xl md:text-3xl text-yellow-400/90 font-medium mb-6 print:mb-4 tracking-tight">{t('hero.role')}</h2>
                ...
                <div className="grid sm:grid-cols-2 gap-8 print:gap-4 print:grid-cols-4">
                    {skills.map((grp, i) => (
                        <div key={i} className="break-inside-avoid">
                            <h4 className="text-lg font-bold text-foreground mb-3 border-b border-muted/50 pb-2 print:mb-1.5 print:pb-1 print:text-sm print:border-gray-300">{grp.category}</h4>
            ...
            <section className="mb-16 print:mb-10">
                ...
                <div className="space-y-12 border-l border-muted ml-1 pl-8 print:space-y-6 print:pl-6 print:border-gray-200">
                    {experiences.map((exp, i) => (
                        <div key={i} className="relative break-inside-avoid group">
                            {/* Dot alignment */}
                            <div className="absolute -left-[37px] top-2 h-2.5 w-2.5 rounded-full bg-yellow-400 ring-4 ring-background group-hover:ring-yellow-400/20 transition-all duration-300 print:bg-gray-600 print:ring-0 print:-left-[29px]" />
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-baseline justify-between mb-2 print:mb-0.5">
                                ...
                                <span className="text-sm font-mono text-muted-foreground font-semibold print:text-gray-500 bg-muted/30 px-2 py-0.5 rounded print:bg-transparent print:p-0">{exp.date}</span>
                            </div>
                            <p className="text-yellow-500/90 font-medium text-sm mb-4 uppercase tracking-wide print:text-gray-700 print:mb-2">{exp.role}</p>
                            <ul className="space-y-2 text-muted-foreground/90 leading-relaxed print:space-y-1">
                                {exp.bullets.map((b, idx) => (
                                    <li key={idx} className="flex items-start gap-3 text-base md:text-lg print:text-sm">
                                        <span className="mt-2.5 w-1.5 h-1.5 rounded-full bg-yellow-400/60 shrink-0 print:bg-gray-400 print:mt-1.5"></span>
                                        {b}
                                    </li>
                                ))}
                            </ul>
              ...
              <div className="grid md:grid-cols-2 gap-8 print:gap-4 print:grid-cols-2">
                {projects.map((proj, i) => (
                  <a 
                    ...
                    className="group flex flex-col h-full rounded-2xl overflow-hidden bg-card border border-muted/50 hover:border-yellow-400/50 transition-all duration-300 hover:shadow-2xl hover:shadow-yellow-400/5 print:bg-white print:border-gray-200 print:shadow-none print:hover:border-gray-200"
                  >
                    ...
                    <div className="p-6 flex-1 flex flex-col print:p-4">
                      ...
                      <div className="mt-auto flex flex-wrap gap-2">
                        {proj.tech.map((techItem: string, idx: number) => (
                          <span key={idx} className="text-xs font-medium px-2.5 py-1 rounded-full bg-muted text-muted-foreground transition-colors group-hover:bg-yellow-400/10 group-hover:text-yellow-500 print:bg-gray-100 print:text-gray-700 print:border-gray-200 print:px-2 print:py-0.5">
                            {techItem}
                          </span>
                        ))}
                      </div>
