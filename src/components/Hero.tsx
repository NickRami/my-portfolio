...
import { ArrowRight, Github, Code2, Server, Database, Cloud, FileUser } from "lucide-react";
    ...
    <section id="hero" className="min-h-screen flex flex-col justify-center items-center px-6 md:px-12 lg:px-24 pt-32 pb-20 max-w-screen-2xl mx-auto relative overflow-hidden">
      ...
      >
        <div className="space-y-4">
          <h1 className="font-display text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight leading-[0.9] text-balance">
            <span className="bg-clip-text text-transparent bg-gradient-to-br from-foreground to-foreground/50">
              {t('hero.role')}
            </span>
          </h1>
          <p className="font-display text-yellow-400/90 text-2xl md:text-3xl font-medium tracking-wide">
            {t('hero.subtitle')}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 justify-center pt-8">
          <a href="#contact" className="px-8 py-3.5 rounded-full bg-foreground text-background font-medium hover:bg-yellow-400 hover:text-black transition-all shadow-lg hover:shadow-yellow-400/20 flex items-center gap-2">
            {t('hero.cta.contact')} <ArrowRight size={18} />
          </a>
          {/* Link to Web Resume instead of PDF Download */}
          <a href={language === 'es' ? "/curriculum" : "/resume"} className="px-8 py-3.5 rounded-full border border-muted-foreground/20 hover:bg-muted/10 hover:border-foreground/50 transition-colors flex items-center gap-2 backdrop-blur-sm">
            <FileUser size={20} /> {language === 'es' ? 'Ver Curriculum' : 'View Resume'}
          </a>
          <a href="https://github.com/NickRami" target="_blank" rel="noopener noreferrer" className="p-3.5 rounded-full border border-muted-foreground/20 hover:bg-muted/10 hover:border-foreground/50 transition-colors flex items-center justify-center backdrop-blur-sm" title="GitHub">
            <Github size={20} />
          </a>
        </div>
      </motion.div>
